...
function tipColor(rgbObj1, rgbObj2) {
	if (rgbObj1 == null || rgbObj2 == null) {
 	...
 	avg_R = Math.floor(rgbObj1['r'] * 0.7 + rgbObj2['r'] * 0.3);
	avg_G = Math.floor(rgbObj1['b'] * 0.7 + rgbObj2['b'] * 0.3);	
	avg_B = Math.floor(rgbObj1['g'] * 0.7 + rgbObj2['g'] * 0.3);
	...
	for (i = 0; i < 19; i++) {
		...
		for (j = 0; j < 19; j++) {
			...
			if (i == 0 && j == 0) {
				...
				cell.style.backgroundColor = COLOR_LIST[j - 1];
				cell.id = "type";
			} else if (j == 0) {
				img.setAttribute("src", PATH2 + getTypePath(i));
				cell.style.backgroundColor = COLOR_LIST[i - 1];

			} else {
				poke_path = getPokePath(i, j, gen);
				if (poke_path != "") {
					img.setAttribute("src", PATH + poke_path);
				}
				i_color = hexToRgb(COLOR_LIST[j - 1]);
				j_color = hexToRgb(COLOR_LIST[i - 1]);
				average_color = tipColor(j_color, i_color);
				r = average_color['r'];
				b = average_color['b'];
				g = average_color['g'];
...
var generation_select = document.getElementById("generationSelect");
generation = generation_select.value;
createTable(generation);

generation_select.addEventListener("change", function() {
